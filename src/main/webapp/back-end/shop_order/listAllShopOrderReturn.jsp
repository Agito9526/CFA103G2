...
<%
	...
	List<Shop_OrderBackVO> list = soSvc.getAllOrderReturn();
...
<body>
	...
	<table id="table-1" align="center">
		<tr>
			<td>
				<h3>所有退貨訂單資料 - listAllShopOrder.jsp</h3> <!-- 		 <h4><a href="select_page.jsp"><img src="images/back1.gif" width="100" height="32" border="0">回首頁</a></h4> -->
			</td>
		</tr>
	</table>
	...
	<table align="center">
		...
		<c:forEach var="Shop_OrderBackVO" items="${list}">
			<tr>
				...
				<td>${Shop_OrderBackVO.shop_return_apply}</td>
				<td>${Shop_OrderBackVO.shop_return_apply_date}</td>
				<td>${Shop_OrderBackVO.shop_return_reason}</td>
				<td>${Shop_OrderBackVO.shop_return_confirm}</td>
				<td>${Shop_OrderBackVO.shop_return_message}</td>
				<td>${Shop_OrderBackVO.shop_return_rej_reason}</td>
				<td>${Shop_OrderBackVO.shop_return_status}</td>
				<td>
				...
				<td>
			  		<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/shop_order/shop_order.do" style="margin-bottom: 0px;">
